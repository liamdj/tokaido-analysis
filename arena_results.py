...
def parse_game_result(file_path):
    ...
    try:
        with open(file_path) as file:
            ...
            last_move = next(
                move["data"] for move in reversed(replay) if move["move_id"] != None
            )
            ...
            for index in range(len(results)):
                placement = (
                    sum(
                        1 if ranks[index] < r else 0.5 if ranks[index] == r else 0
                        for r in ranks
                    )
                    - 0.5
                )
                ...
                outcomes[id] = (
                    start_infos[id]["traveler"],
                    int(start_infos[id]["player_position_order"]),
                    placement,
                    score,
                    achievements,
                )
...
def create_results_summary(tables, path):
    ...
    Result = namedtuple(
        "Result",
        ["table_id", "traveler", "position", "placement", "points", "achievements"],
    )
    results = []
    for table_id, players in tables.items():
        outcomes = parse_game_result("replays/{}.json".format(table_id))
        if outcomes:
            results.extend(
                [
                    Result(table_id, res[0], res[1], res[2], res[3], res[4])
                    for id, res in outcomes.items()
                    if id in players
                ]
            )
